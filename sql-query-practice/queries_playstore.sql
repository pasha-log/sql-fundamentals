...
SELECT category, COUNT(category = category) FROM analytics GROUP BY category;
...
SELECT app_name FROM analytics WHERE price > .10 AND price < 1 ORDER BY reviews DESC LIMIT 10;
...
SELECT app_name, reviews, min_installs, MAX(min_installs/reviews) AS proportion FROM analytics WHERE min_installs >= 100000 GROUP BY app_name, reviews, min_installs ORDER BY MAX(min_installs/reviews) DESC LIMIT 1;